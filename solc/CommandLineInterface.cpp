...
bool CommandLineInterface::checkMutualyExclusive(std::string const& _optionA, std::string const& _optionB) const
{
	if (m_args.count(_optionA) && m_args.count(_optionB))
	{
		serr() << "Option " << _optionA << " and " << _optionB << " are mutualy exclusive." << endl;
		return false;
	}
...
General Information)").c_str(),
	...
	if (!checkMutualyExclusive(g_argColor, g_argNoColor))
		return false;
	...
	for (auto& option: conflictingWithStopAfter)
		if (!checkMutualyExclusive(g_strStopAfter, option))
			return false;
